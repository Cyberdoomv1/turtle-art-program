...
Drawing:
dot, circle/circ/cir, centrecircle/ccircle/ccir, arc/ar/a, polygon/poly, clear/clearscreen, undo/und/un
...
def save(toggle1,saving1,rawinput):
    ...
    if toggle == True:
        ...
        if saving == False:
            ...
            validating = True
            while validating:
                fname = input("Please enter the name your photo should be saved under (will append to existing files): ")
                print(fname)
                ...
                if choice in("y","ye","yes"):
                    ...
                    validating = True
            print("will append to a file with this name")
            # File is now set up.
            saving = True
        if saving == True:
            # If saving is already set up then save.
            # Will also happen when calling save for a second time.
            if rawinput.lower() not in("save,true","save,false"):
                try:
                    with open(fname,"a") as file:
                        print(rawinput + " saving")
                        file.write(rawinput+"\n")
                except Exception as e:
                    print(e)
    if toggle == False:
