...
def save(toggle1,saving1,rawinput):
    global saving
    global toggle
    global fname
    global appending
    ...
    if toggle == True:
        ...
        if saving == False:
            ...
            validating = True
            while validating:
                fname = input("Please enter the name your photo should be saved under (will overwrite existing files): ")
                print(fname)
                choice = input("is this the name you want? y/n: ").lower()
                if choice in("y","ye","yes"):
                    print("File Name set to",fname)
                    validating = False
                elif choice in("n","no"):
            ...
            while validating:
                validating2 = True
                while validating2:
                    appending = input("do you wish to add onto an already existing file with this name? y/n: ")
                    print("you have chosen",appending)
                    if appending in("y","ye","yes"):
                        appending = "a"
                        validating2 = False
                    elif appending in("n","no"):
                        appending = "w"
                        validating2 = False
                    else:
                        print("Invalid input try again")
                ...
                if choice in("y","ye","yes"):
                    if appending == "a":
                        print("will append to",fname)
                    else:
                        print("will overwrite file with this name")
                    validating = False
                elif choice in("n","no"):
            ...
            saving = True
        if saving == True:
            # If saving is already set up then save.
            # Will also happen when calling save for a second time.
            try:
                with open(fname,appending) as file:
                    file.write(rawinput)
            except Exception as e:
                print(e)
    if toggle == False:
        ...
        print("type true or false next to save to toggle whether you are saving")
...
def do_command(rawinput,command,empty,empty2,empty3,empty4,empty5,empty6,saving,toggle):
    try:
        #-------------------------------------------------------------------------#
        #colour stuff
        #-------------------------------------------------------------------------#
...
def inputstuff(rawinput,command,empty,empty2,empty3,empty4,empty5,empty6,fileexec,saving,toggle):
    try:
    #is where commands are input
        if fileexec == False:
            rawinput = input("Command: ")
        if toggle == True:
            save(toggle,saving,rawinput)
        empty = rawinput
        if empty[-1] == " ":
            empty = empty[0:-1]
        if empty[0] == " ":
        ...
        do_command(rawinput,command,empty,empty2,empty3,empty4,empty5,empty6,saving,toggle)
...
while play:
    inputstuff(rawinput,command,empty,empty2,empty3,empty4,empty5,empty6,fileexec,saving,toggle)
