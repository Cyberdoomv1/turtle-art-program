#v1.9.1
#added guizero to add a GUI but its barely functional and all i have donme is move where you type the commands
import turtle
import guizero as g
...
def filecommand(empty,rawinput):
    try:
        with open(empty,"r") as file:
            for line in file:
                ...
                print(line.strip())
                rawinput = line.strip()
                inputstuff(rawinput,command,empty,empty2,empty3,empty4,empty5,empty6,fileexec)
            fileexec = False
    except Exception as e:
        print(e)
...
def show_var_boxes():
    global toggle_var_boxes
    if toggle_var_boxes == 0:
        toggle_var_boxes = 1
        instruction3 = g.Text(art_program, text = "argument 1")
        empty = g.TextBox(art_program)
        instruction4 = g.Text(art_program, text = "argument 2")
        empty2 = g.TextBox(art_program)
        instruction5 = g.Text(art_program, text = "argument 3")
        empty3 = g.TextBox(art_program)
        instruction6 = g.Text(art_program, text = "argument 4")
        empty4 = g.TextBox(art_program)
        instruction7 = g.Text(art_program, text = "argument 5")
        empty5 = g.TextBox(art_program)
        instruction8 = g.Text(art_program, text = "argument 6")
...
def inputstuff(rawinput,command,empty,empty2,empty3,empty4,empty5,empty6,fileexec):
    try:
    #is where commands are input
        instruction1 = g.Text(art_program, text = "Enter your command")
        if fileexec == False:
            rawinput = rawinputbox.value
        ...
        do_command(rawinput,command,empty,empty2,empty3,empty4,empty5,empty6)
...
while play:
    instruction2 = g.Text(art_program, text = "this box is for command line style inputs and will override other boxes")
    rawinputbox = g.TextBox(art_program)
    ind_var_choice = g.PushButton(art_program, command = show_var_boxes ,text = "individual argument choice")
    exe = g.PushButton(art_program, command = inputstuff, text = "execute", args=[rawinput,command,empty,empty2,empty3,empty4,empty5,empty6,fileexec])
    art_program.display()
